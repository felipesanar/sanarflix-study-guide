...
export function exportAnalyticsCSV(data: AnalyticsExportData, filters: AnalyticsExportFilters): void {
  ...
  const lines: string[] = [
    ...
    '# ═══════════════════════════════════════════════════════════════════',
    '',
    'Matéria,Progresso (%),Aulas Concluídas,Aulas Disponíveis',
    ...data.progress.progressoMedioPorMateria.map(m => `"${m.materia}",${m.progresso},${m.aulasConcluidas},${m.aulasDisponiveis}`),
    '',
    '# ═══════════════════════════════════════════════════════════════════',
    '# PROGRESSO - POR FAIXA',
    '# ═══════════════════════════════════════════════════════════════════',
...
export function exportAnalyticsXLSX(data: AnalyticsExportData, filters: AnalyticsExportFilters): void {
  ...
  const progressoData = [
    ...
    ['PROGRESSO POR MATÉRIA'],
    ['Matéria', 'Progresso (%)', 'Aulas Concluídas', 'Aulas Disponíveis'],
    ...data.progress.progressoMedioPorMateria.map(m => [m.materia, m.progresso / 100, m.aulasConcluidas, m.aulasDisponiveis]),
    [''],
    [''],
