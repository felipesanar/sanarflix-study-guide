            ...
            <div className="space-y-6 py-4">
              ...
              {question.imagem && (
                <div className="flex justify-center my-4">
                  <img
                    src={normalizeImageUrl(question.imagem)}
...
const EvolutionChart: React.FC<{ allPerformanceData: any[] }> = ({ allPerformanceData }) => {
  const evolutionData = useMemo(() => {
    ...
    allPerformanceData.forEach(item => {
      const sid = String(item.simulado_id);
      if (!simuladoNames[sid]) simuladoNames[sid] = item.simulado_nome;
      ...
      area[`simulado_${sid}`] = percentual;
    });
    const data = Array.from(areasMap.values());
    const simulados = Object.entries(simuladoNames)
      .map(([id, name]) => ({ id, name }))
  ...
  if (evolutionData.data.length === 0 || evolutionData.simulados.length < 2) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><ChevronsUpDown className="h-5 w-5 text-primary" />Evolução entre Simulados</CardTitle>
        </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><ChevronsUpDown className="h-5 w-5 text-primary" />Evolução entre Simulados por Grande Área</CardTitle>
      </CardHeader>
      <CardContent className="h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          <RechartsBarChart data={evolutionData.data} margin={{ top: 30, right: 20, left: 0, bottom: 5 }}>
            <XAxis dataKey="name" tick={{ fontSize: 12, fill: 'hsl(var(--muted-foreground))' }} axisLine={{ stroke: 'hsl(var(--border))' }} tickLine={false} />
            <YAxis domain={[0, 100]} tick={{ fill: 'hsl(var(--muted-foreground))' }} axisLine={{ stroke: 'hsl(var(--border))' }} tickFormatter={(value) => `${value}%`} />
            <Tooltip cursor={{ fill: 'hsl(var(--accent))' }} contentStyle={{ backgroundColor: 'hsl(var(--background))', borderColor: 'hsl(var(--border))', borderRadius: '0.5rem' }} />
            <Legend wrapperStyle={{ fontSize: '14px' }} />
            {evolutionData.simulados.map((simulado, index) => (
              <Bar key={simulado.id} dataKey={`simulado_${simulado.id}`} name={simulado.name} fill={dynamicColors[index]} radius={[4, 4, 0, 0]} label={<RenderCustomEvolutionBarLabel />} />
            ))}
          </RechartsBarChart>
        </ResponsiveContainer>
      </CardContent>
...
export const SimuladoDesempenho: React.FC = () => {
  ...
  const fetchDataForView = async (simuladoId: string | null, forceRefresh = false) => {
    ...
    try {
      ...
      setSimulados((simuladosData as any[]).map(s => ({ id: String(s.id), nome: s.nome })) as Simulado[]);
    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        questions = data.map((q: any) => ({
          ...
          acertou: q.correct === true,
        }));
      } else {
        const { data: qsData } = await supabase
          .from('questoes_simulado')
          .select('id, enunciado, alternativa_a, alternativa_b, alternativa_c, alternativa_d, correta, comentario, imagem')
          .eq('simulado_id', selectedSimulado as string)
          .order('ordem', { ascending: true })
          .limit(10) as any;

        const raw = (qsData as any[]) || [];
        const ids = raw.map((q: any) => q.id);
        let correctnessMap: Record<string, boolean> = {};
        if (user?.email && ids.length > 0) {
          const { data: ans } = await supabase
            .from('answer_progress')
            .select('question_id, correct')
            .eq('email', user.email)
            .eq('simulado', selectedSimulado as string)
            .in('question_id', ids);
          if (ans) {
            correctnessMap = (ans as any[]).reduce((acc: Record<string, boolean>, r: any) => {
              acc[r.question_id] = (acc[r.question_id] || false) || !!r.correct;
              return acc;
        ...
        questions = raw.map((q: any) => ({
          ...
          imagem: normalizeImageUrl(q.imagem),
          dificuldade: 'Médio',
          acertou: correctnessMap[q.id] === true,
        }));
      }
