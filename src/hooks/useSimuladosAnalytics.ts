...
// Exported for direct use (e.g., in export modal without visiting Simulados tab)
export async function fetchSimuladosAnalyticsData(
  filters: SimuladosFilters,
  signal?: AbortSignal
): Promise<Omit<SimuladosAnalyticsData, 'isLoading' | 'error'>> {
  ...
  // PHASE 3: Fetch answers and history
  ...
  // Update cache
  analyticsCache = {
    data: { ...result, isLoading: false, error: null },
    timestamp: Date.now(),
    filterKey: cacheKey,
  };
...
export function useSimuladosAnalytics(filters: SimuladosFilters) {
  ...
  const fetchData = useCallback(async (skipCache = false) => {
    ...
    try {
      // Use the standalone fetch function
      const result = await fetchSimuladosAnalyticsData(filters, abortControllerRef.current.signal);
