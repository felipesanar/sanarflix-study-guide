  ...
  try {
    ...
    for (const simulado of simuladosToNotify) {
      ...
      const { data: alreadyNotified, error: notifiedError } = await supabase
        .from('performance_notifications_sent')
        .select('user_id')
        .eq('simulado_id', simulado.id)
        .in('user_id', userIds);
      ...
      const notifiedUserIds = new Set((alreadyNotified || []).map((n: any) => n.user_id));
      const usersToNotifyForSimulado = userIds.filter((uid: string) => !notifiedUserIds.has(uid));

      if (usersToNotifyForSimulado.length === 0) {
        continue;
      }
      ...
      const { data: users, error: usersError } = await supabase
        ...
        .in('id', usersToNotifyForSimulado);
      ...
      try {
                        ...
                        <tr>
                          <td style="padding: 40px 30px;">
                            ...
                            <p style="color: #374151; font-size: 16px; line-height: 1.6; margin: 0 0 30px;">
                              Acesse a plataforma para ver seus resultados detalhados.
                            </p>
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            simulado_id: user.simulado_id
          });

        if (insertError) {
          console.error(`Error recording notification for ${user.email}:`, insertError);
        } else {
          console.log(`[notify-performance-released] Notification recorded for ${user.email}`);
        }
      } catch (emailError) {
        console.error(`[notify-performance-released] Failed to process ${user.email}:`, emailError);
      }
    }
