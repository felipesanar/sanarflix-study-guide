-- get_gestor_detalhamento: acrescenta a chave `alunos` ao envelope.
--
-- POR QUE: o bloco "Visao de alunos" do Detalhamento nascia permanentemente
-- vazio. A rota le `dados.alunos`, mas NENHUMA versao de
-- get_gestor_detalhamento jamais emitiu essa chave -- conferido com
-- `grep "'alunos'" supabase/migrations/*.sql`. Como o campo e opcional no tipo
-- (`DetalhamentoComExtras`), o TypeScript ficava mudo, e `dados.alunos ?? []`
-- fazia a tabela AFIRMAR "Nenhum aluno neste recorte / 0 participantes" para
-- uma IES com centenas de alunos que participaram do simulado. A suite passava
-- porque o fixture de teste fabricava a chave que o servidor nunca mandou.
--
-- O QUE MUDA, exatamente: o corpo abaixo e o corpo VIGENTE (dump de producao
-- gravado por 20260806144647_gestor_remove_guard_portal_v2_ga_total.sql) com
-- TRES adicoes e nenhuma remocao:
--   1. a CTE `alunos` passa a trazer `u.nome` alem de `u.id, u.semestre`;
--   2. tres CTEs novas -- `alvo`, `anterior`, `aluno_linha` -- montadas so a
--      partir de CTEs que ja existiam (`sims_ord`, `alunos`, `tentativas`,
--      `respostas`, `tri`). Nenhuma tabela nova entra na consulta;
--   3. a chave `alunos` no objeto `data`.
-- Guard `gestor_pode_acessar_ies`, SECURITY DEFINER, search_path, o recorte por
-- semestre (`v_sems`), o dedupe `DISTINCT ON` do TRI e os desempates ficam
-- literalmente intactos -- este arquivo foi GERADO a partir do corpo anterior,
-- nao redigitado.
--
-- ANTES DE APLICAR (mesmo porteiro das outras):
--   SELECT pg_get_functiondef('public.get_gestor_detalhamento(uuid,text,uuid[])'::regprocedure);
-- Compare com o corpo assumido, que e o de 20260806144647. Divergiu, ABORTA e
-- chama -- foi assim que se descobriu a admin_set_simulado_agenda dropada.
--
-- DEPOIS DE APLICAR: remover da rota o estado "Lista de alunos indisponivel"
-- (routes/Detalhamento.tsx). O teste `contratoEnvelopeRpc.test.ts` cobra isso
-- sozinho: ele le as chaves emitidas por esta funcao e, quando `alunos` passa a
-- existir, exige que a rota pare de tratar o caso `undefined`.
