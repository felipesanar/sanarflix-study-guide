-- A4 (plano 2026-08-09-gestor-v2-correcoes-e-features.md): drill-down de área
-- na tela "Detalhamento", que recorta por p_simulados (array explícito) em
-- vez de p_semestre -- por isso não reaproveita get_gestor_diagnostico /
-- get_gestor_diagnostico_temas (ambas recortam por p_ies_id + p_semestre).
--
-- Combina:
--  - a lógica de agregação/classificação de get_gestor_diagnostico (níveis
--    grande_área -> especialidade -> tema, classificação crítico/mediano/
--    excelente sobre o mesmo valor arredondado exposto em acertoPct,
--    temFilhos indicando se há um próximo nível a explorar);
--  - o padrão de recorte por p_simulados de get_gestor_detalhamento (scope
--    check dos ids recebidos, grupo por simulado_pai_id, fallback de
--    tentativa via simulados_finalizados/answer_progress).
--
-- Contrato: p_grande_area é obrigatório (mesmo contrato de
-- get_gestor_diagnostico_temas). Sem p_especialidade, agrupa por
-- especialidade dentro da grande área (nível "especialidade"); com
-- p_especialidade, agrupa por tema dentro da especialidade (nível "tema",
-- sempre folha -> temFilhos = false).
...
BEGIN
  ...
  -- de get_gestor_diagnostico_temas.
  ...
  nos AS (
    SELECT a.nome,
           a.acerto_pct,
           a.amostra,
           -- classifica sobre a MESMA base arredondada que sai em
           -- acertoPct -- nunca sobre a razão bruta (mesmo cuidado de
           ...
           -- só o nível "especialidade" tem próximo nível (tema); tema é
           -- sempre folha nesta RPC.
           CASE
