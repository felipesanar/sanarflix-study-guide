...
-- ============================================================
-- Update all institutional RPCs to support b2b_partner role
-- b2b_partner and admin can filter by any IES via p_ies_id
-- professor uses their own IES only
-- ============================================================

-- RPC 1: get_institutional_simulados (now accepts optional p_ies_id)
CREATE OR REPLACE FUNCTION public.get_institutional_simulados(p_ies_id uuid DEFAULT NULL)
...
BEGIN
  v_user_id := auth.uid();
  IF NOT (has_role(v_user_id, 'admin') OR has_role(v_user_id, 'professor') OR has_role(v_user_id, 'b2b_partner')) THEN
    RAISE EXCEPTION 'Access denied: requires admin, professor or b2b_partner role';
  END IF;
  
  -- admin/b2b_partner can specify any IES; professor uses own IES
...
-- RPC 2: get_institutional_performance (now accepts optional p_ies_id)
CREATE OR REPLACE FUNCTION public.get_institutional_performance(p_simulado_id uuid, p_ies_id uuid DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
-- RPC 3: get_institutional_question_details (now accepts optional p_ies_id)
CREATE OR REPLACE FUNCTION public.get_institutional_question_details(
  p_simulado_id uuid, p_tema text, p_area text DEFAULT NULL, p_specialty text DEFAULT NULL, p_ies_id uuid DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
STABLE SECURITY DEFINER
...
-- RPC 4: get_institutional_student_scores (now accepts optional p_ies_id)
CREATE OR REPLACE FUNCTION public.get_institutional_student_scores(p_simulado_id uuid, p_ies_id uuid DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
-- RPC 5: get_institutional_evolution (now accepts optional p_ies_id)
CREATE OR REPLACE FUNCTION public.get_institutional_evolution(p_ies_id uuid DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
-- RLS policies for b2b_partner
CREATE POLICY "B2B partners can view all answers"
ON public.answer_progress FOR SELECT TO authenticated
USING (has_role(auth.uid(), 'b2b_partner'));

CREATE POLICY "B2B partners can view all users"
ON public.users FOR SELECT TO authenticated
USING (has_role(auth.uid(), 'b2b_partner'));

CREATE POLICY "B2B partners can view all simulados"
ON public.simulados_admin FOR SELECT TO authenticated
USING (has_role(auth.uid(), 'b2b_partner'));

CREATE POLICY "B2B partners can view all questoes"
