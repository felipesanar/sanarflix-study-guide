-- Fase 2 (pos GA total): remove o guard de feature 'gestao.portal_v2' das RPCs
-- get_gestor_* e de get_gestor_contexto, e apaga o dado morto correspondente em
-- feature_catalog/ies_features.
--
-- Contexto: o commit "GA total" (58226452) removeu o gate no front-end -- todo
-- gestor passou a ver o Portal do Gestor v2 direto, sem checar feature nenhuma.
-- Mas so tocou .tsx: as 10 RPCs get_gestor_* e get_gestor_contexto continuaram
-- checando 'gestao.portal_v2' no banco (via user_has_feature_for_ies ou
-- user_has_feature). Como so a IES FAI tinha essa chave = true, as outras 13
-- IES com gestao.enabled = true passariam a receber 'feature_not_enabled' ao
-- abrir o portal, em vez da tela antiga -- pior do que o comportamento antes do
-- GA total. Decisao do Felipe (06/08): tirar o guard, a v2 vale para todo
-- mundo, sem checagem nem no front nem no banco.
--
-- Apos a remocao, 'gestao.portal_v2' e as 5 chaves por modulo
-- (gestao.visao_institucional, gestao.diagnostico_curricular, gestao.alunos,
-- gestao.insights_pedagogicos, gestao.inteligencia_decisoria) finalmente ficam
-- dado morto de verdade -- apagadas de feature_catalog/ies_features no final
-- desta mesma migration.
--
-- public.user_has_feature_for_ies(text, uuid) fica ORFA depois desta migration
-- (nenhum outro chamador em supabase/migrations/*.sql alem deste guard, achado
-- 2 de 03/08). Mantida de proposito -- nao e removida aqui, so sinalizada.
-- public.user_has_feature(text) NAO e tocada: e usada por 19 RPCs legadas para
-- outras chaves (aluno.%, gestao.enabled, gestao.diagnostico_curricular).
...
-- Dado morto: as 6 chaves de feature do Portal do Gestor v2 nao tem mais
-- nenhum guard as protegendo (nem front-end, nem banco). Removidas de
...
'Checa se uma feature de ies_features esta ligada PARA UMA IES ESPECIFICA (master gestao.enabled + chave), sem bool_or sobre as IES do grupo. ORFA desde a migration que removeu o guard de gestao.portal_v2 das 10 RPCs get_gestor_* (GA total, 06/08) -- nenhum chamador em supabase/migrations/*.sql alem daquele guard. Mantida de proposito, nao dropada.';
