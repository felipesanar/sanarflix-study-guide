-- Gestor v2, Fase 5, Task 52: get_gestor_detalhamento passa a devolver, dentro
-- de acertoPorAreaESemestre, o campo aditivo `matriz` (área x semestre) --
-- necessário para o recorte cruzado no cliente, sem round-trip (transição de
-- 200ms no valor). Reaproveita a CTE `respostas` já existente na função
-- (mesma fonte/filtro/dedupe de `areas` e `semestres`, só agrupando pelos
-- dois eixos juntos) -- nenhum join novo, nenhuma outra parte da função muda.
--
-- Campo `alunos` (para a Task 53 / TabelaAlunosSimulado) fica para quando
-- essa Task entrar em escopo -- não faz sentido tocar produção por um campo
-- que nenhum componente vai ler ainda.
