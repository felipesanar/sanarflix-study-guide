-- Lote E (06/08): reaplica o guard de feature 'gestao.enabled' (master) em
-- get_gestor_detalhamento e get_gestor_questoes, que o perderam num merge.
--
-- O QUE ACONTECEU
-- ----------------
-- A migration 20260806180000 (Lote D) restaurou o guard de 'gestao.enabled'
-- nas 11 RPCs get_gestor_*, incluindo estas duas. Um merge da main (PR #18)
-- trouxe a migration 20260806192302_3812e63d-9666-4524-9cd7-1968f6d69d92.sql,
-- que recria get_gestor_detalhamento e get_gestor_questoes por inteiro SEM
-- nenhum guard de feature -- trazendo, junto, as mudancas de "alunos e
-- whitelist" que vieram da main (fora do escopo desta correcao). Como o
-- timestamp da 192302 e POSTERIOR ao da 180000, o CREATE OR REPLACE FUNCTION
-- mais novo prevaleceu e apagou o guard destas duas, em silencio -- sem
-- nenhum erro, nenhum teste de tipo, nada que avisasse em compile-time. As
-- outras 9 RPCs do Lote D nao foram tocadas pelo merge e continuam com o
-- guard.
--
-- A CORRECAO -- SOMENTE o guard volta, corpo igual ao vigente
-- ---------------------------------------------------------------
-- Esta migration recria SOMENTE estas duas funcoes, usando como base o corpo
-- vigente de cada uma (20260806192302 -- a mais recente que as recria, ja com
-- as mudancas da main), acrescentando de volta o MESMO bloco de guard de
-- 'gestao.enabled' que o Lote D ja usava para as duas. Nenhuma outra linha
-- muda: fora do bloco de guard, o corpo e identico ao da 192302. Nao traz de
-- volta 'gestao.portal_v2' nem as 5 chaves por modulo -- essas continuam
-- mortas.
--
-- POSICAO DO GUARD -- MESMA DO LOTE D, NAO IMPROVISAR
-- --------------------------------------------------------
...
-- p_ies_id vem NULL. Como user_has_feature_for_ies e fail-closed para v_ies
-- NULL, um guard cedo faria toda chamada sem IES explicita estourar
-- 'feature_not_enabled' -- armadilha documentada em
-- 20260804120000_user_has_feature_for_ies.sql:99-127 e repetida no cabecalho
-- da 20260806180000. Em ambas as funcoes o guard fica, nesta ordem: papel
-- (Access denied) -> resolucao de v_ies (IES not resolved) -> autorizacao da
-- IES (gestor_pode_acessar_ies) -> feature (feature_not_enabled). O guard
-- recebe SEMPRE v_ies, nunca p_ies_id.
--
-- RESTRICOES DESTA MIGRATION
-- ---------------------------
-- Aditiva: so CREATE OR REPLACE FUNCTION destas 2, nenhum DROP/DELETE/
-- INSERT/ALTER. Nao recria public.user_has_feature nem
-- public.user_has_feature_for_ies -- a primeira e compartilhada com 19 RPCs
-- legadas e seu corpo real nao existe em nenhum .sql do repo (guard
-- injetado pela migration 20260709171344); um CREATE OR REPLACE nela
-- apagaria esse guard em silencio. Sem REVOKE/GRANT: CREATE OR REPLACE
-- preserva privilegios existentes, repeti-los aqui seria inocuo mas
-- desnecessario.
--
-- QUEM RECRIAR QUALQUER UMA DAS 11 PRECISA REINSERIR O GUARD
-- -----------------------------------------------------------
-- O guard vive DENTRO do corpo de cada uma das 11 funcoes get_gestor_*, nao
-- numa trigger ou policy separada que sobreviveria a um CREATE OR REPLACE.
-- Isso ja aconteceu duas vezes: a 20260806144647 (GA total) apagou o guard
-- de 'gestao.portal_v2' + 'gestao.enabled' das 11 sem intencao ao limpar
-- portal_v2; e esta migration corrige o mesmo efeito colateral, desta vez
-- causado pela 192302 (merge da main) sobre so 2 das 11. Quem recriar
-- get_gestor_contexto, get_gestor_aluno_contato, get_gestor_cronograma,
-- get_gestor_avisos, get_gestor_visao_geral, get_gestor_diagnostico,
-- get_gestor_diagnostico_temas, get_gestor_alunos, get_gestor_aluno, ou as
-- duas desta migration (get_gestor_detalhamento, get_gestor_questoes) por
-- QUALQUER motivo -- fix, feature nova, refactor, merge -- precisa
-- reinserir o mesmo bloco de guard 'gestao.enabled' no novo corpo, na mesma
-- posicao (depois da autorizacao por IES), ou ele desaparece de novo.
--
-- STATUS DE APLICACAO -- NAO CONFUNDIR ARQUIVO COM BANCO
-- ---------------------------------------------------------
-- Esta migration NAO FOI APLICADA em producao (escrita em 06/08/2026).
...
-- acontecer de verdade, o servidor CONTINUA sem validar 'gestao.enabled' em
-- get_gestor_detalhamento e get_gestor_questoes, exatamente como a 192302
-- deixou. Quem ler este arquivo e concluir que o guard ja esta ativo em
-- producao esta lendo o repo, nao o banco -- confirme o estado real antes
-- de assumir qualquer coisa sobre modulo contratado.
